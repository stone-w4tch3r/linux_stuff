...
def deploy_ssh() -> None:
    ...
    python.call(function=lambda: assert_user_exists(server_user))

    if overwrite or open(local_known_hosts).read().find(host.name) == -1 or not os.path.isfile(ssh_key):
        python.call(function=lambda: generate_and_install_ssh_key_locally(ssh_key, local_known_hosts))
    ...
    python.call(function=lambda: update_connection_data(ssh_key, server_user, server_user_password))
